...
class Footstep {
    ...
    class func fetchFootstepUsers(completion: @escaping(Footstep) -> Void) {
        ...
        Block.fetchBlockSwipe { (blockUserIDs) in
            COLLECTION_FOOTSTEP.document(User.currentUserId()).collection(ISFOOTSTEP).order(by: DATE).limit(to: 5).getDocuments { (snapshot, error) in
                if let error = error {
    ...
    class func fetchFootstepedUsers(completion: @escaping(Footstep) -> Void) {
        ...
        Block.fetchBlockSwipe { (blockUserIDs) in
            COLLECTION_FOOTSTEP.document(User.currentUserId()).collection(FOOTSTEPED).order(by: DATE).limit(to: 5).getDocuments { (snapshot, error) in
                if let error = error {
                    print("Error: \(error.localizedDescription) ")
                }
